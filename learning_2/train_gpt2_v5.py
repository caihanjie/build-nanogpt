...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        if master_process:
            print(f"number of parameters tensors :{len(decay_params)}) with {num_decay_params} elements to decay")
            print(f"number of parameters tensors :{len(nodecay_params)}) with {num_nodecay_params} elements to not decay")

        fused_available = 'fused' in inspect.signature(torch.optim.AdamW).parameters

        used_fused = fused_available
        if master_process:
            print("using fused AdamW:", used_fused)
...
class DataLoaderLite:
    def __init__(self, B, T,process_rank,num_processes):
        ...
        if master_process:
            print(f"dataset length in tokens: {self.tokens.size(0)}")
    ...
    def next_batch(self):
        ...
        if self.current_positions + B*T* self.num_processes + 1 >= self.tokens.size(0):
            self.current_positions = self.B * self.T * self.process_rank
        return x, y

#torchrun --standalone --nproc_per_node=2 learning/train_gpt2_v5.py
...
for step in range(50):
    ...
    t1 = time.time()
    dt = (t1 - t0)*1000 
    tokens_per_sec = train_loader.B * train_loader.T / (t1 - t0) * grad_accum_steps * ddp_world_size
    if master_process:
        print(f"step {step+1}, loss: {loss_accum.item():.4f},lr: {lr:.4e}, norm: {norm:.4f}, time: {dt:.2f} ms, tokens/sec: {tokens_per_sec:.2f}")
